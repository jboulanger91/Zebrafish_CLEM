...
| `load_clem_table.py` | Loads CLEM cell metadata from the cell inventory xlsx (CLEM sheet). Sets function/morphology from xlsx columns, computes `reconstruction_complete` boolean, resolves metadata file paths. | `load_clem_table(xlsx_path) -> DataFrame` | `pandas` |
