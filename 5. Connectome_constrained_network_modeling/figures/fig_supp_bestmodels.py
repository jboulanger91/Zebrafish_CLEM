...
show_matrix_style = ["recorded"]  # ["recorded", "all"] as an alternative to show the full matrix as well
...
model_list = []
# Fetch models
for path_model in path_top_model.glob("model_*.pt"):
...
for i_m, i_m_sorted in enumerate(top_indices):
    ...
    for matrix_style in show_matrix_style:
        ...
        _, _, ypos = RNNService.plot_connectivity(mask_W, U=mask_U, neuron_identity_array=neuron_identity_array,
                                                     ...
                                                     plot_title=f"{matrix_style.capitalize() + '\n'}Mask W",
                                                     cmap=colormap, show_colorbar=i_m == len(top_indices)-1)

        ypos -= plot_size_matrix + padding * 1.5
        _, _, ypos = RNNService.plot_connectivity(W, U=U, neuron_identity_array=neuron_identity_array,
                                                     grid_pop=grid_pop,
                                                     fig=fig, xpos=xpos, ypos=ypos, plot_size_matrix=plot_size_matrix,
                                                     padding=padding, value_lim=[-1, 1], cmap=colormap, show_colorbar=i_m == len(top_indices)-1)
...
fig.save(path_save / f"figure_supp_bestmodels.pdf", open_file=False, tight=style.page_tight)
