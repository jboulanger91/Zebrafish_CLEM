...
if __name__ == "__main__":
    ...
    for i_model in range(n_models):
        ...
        for side in ConfigurationRNN.side_list:
            idx_pop = dict_neurons["neurons"][side][cell_seed]["idx_list"]

            # Compute sparsity
            W_binary_pop = W_binary_model[np.ix_(idx_pop, idx_pop)]
            ...
            recurrency_rate_pop = len(recurrency_pop["rediscovered_group"]) / len(recurrency_pop["presynaptic"] | recurrency_pop["seed"] | recurrency_pop["postsynaptic"])

            while sparsity_pop < target_sparsity_pop and recurrency_rate_pop < target_recurrency_rate_pop:
                ...
                W_binary_model[np.ix_(idx_pop, idx_pop)] = W_binary_pop

                recurrency_pop = analyze_selected_neurons(W_binary_model, idx_pop)
                recurrency_rate_pop = len(recurrency_pop["rediscovered_group"]) / len(recurrency_pop["presynaptic"] | recurrency_pop["seed"] | recurrency_pop["postsynaptic"])

        print(f"Final recurrency LiMI: {recurrency_rate_pop}")
        print(f"Final sparsity LiMI: {sparsity_pop}")
