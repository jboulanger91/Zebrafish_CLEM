...
Drop-in notes
-------------
* `RNNService.plot_response` dispatches on `type(model).__name__`; add
  `"RNNConnectomeV2"` to the `RNNFixedConnectivity` branch (this class exposes
  the same `idx_L*`/`idx_R*` attributes). `plot_response_by_cell` needs nothing.
* `tau` is stored as `self.tau` as well as `self.alpha`, so checkpoints made by
  `RNNService.extract_custom_attrs` round-trip it.
"""
...
class SpectralEstimateError(RuntimeError):
    """
    The dominant eigenvalue of the Jacobian could not be estimated.

    Raised instead of letting a non-finite matrix reach LAPACK: `torch.linalg.eig`
    on a matrix containing inf/nan aborts inside oneMKL (`SGEBAL` parameter
    error) and takes the process down with an access violation rather than an
    exception, so the check has to happen before the call, every time.
    """
...
class RNNConnectome(nn.Module):
    ...
    def __init__(
        ...
        # 0.4 s, so there is no gradient anywhere that says "build a 20 s mode",
        # while there is always a gradient that says "shrink". Observed on this
        # connectome: gain 0.64 -> 0.48 over 25 epochs, monotonically.
        ...
        self.gain_floor_warmup_epochs = int(gain_floor_warmup_epochs)
        self._gain_at_init = None      # set below, once the rescale is done
        ...
        # ---- init report -----------------------------------------------------
        self._gain_at_init = gain_at_unit
        ...
        if self.gain_floor is None:
            ...
            print(f"  gain band: [{self.gain_floor:.4f}, {self.gain_target:.4f}]  -> "
                  ...
                  f"floor ramped in over {self.gain_floor_warmup_epochs} epochs "
                  f"from {gain_at_unit:.4f}")
            if self.gain_floor > 0.99:
                print(f"  NOTE tau_eff_min={self.tau_eff_min} s at tau={self.tau:.3f} s "
                      f"needs gain >= {self.gain_floor:.4f}. That is a demanding "
                      f"floor; raise tau or tau_eff_min if the fit fights it.")
    ...
    @staticmethod
    def _require_finite(*tensors):
        for t in tensors:
            if t is None:
                continue
            if not torch.isfinite(t).all():
                raise SpectralEstimateError("non-finite matrix reached the spectral "
                                            "estimator")
    ...
    def _dominant_pair(self, J_detached):
        ...
        """
        self._require_finite(J_detached)
        lam, V = torch.linalg.eig(J_detached)
        k = int(torch.argmax(lam.abs()).item())
        # `torch.linalg.inv` on a near-defective eigenvector basis returns
        # inf/nan SILENTLY rather than raising -- and a 1.6%-density Dale matrix
        # is non-normal enough to be nearly defective, so this happens in
        # practice. Validate instead of trusting it.
        Vinv = torch.linalg.inv(V)
        u = Vinv[k, :]
        v = V[:, k]
        if not (torch.isfinite(u).all() and torch.isfinite(v).all()
                and torch.isfinite(lam[k])):
            raise SpectralEstimateError("eigenvector basis is numerically singular")
        return lam, k, u, v
    ...
    def dominant_jacobian_eigenvalue(self, W=None, D=None, d_scalar=None, detach=False):
        ...
        if self.spectral_mode == "rayleigh":
            try:
                ...
                if not torch.isfinite(out):
                    raise SpectralEstimateError("Rayleigh quotient was not finite")
                return out, imag_frac
            except SpectralEstimateError:
                raise
        ...
        out = torch.norm(J @ v)
        if not torch.isfinite(out):
            raise SpectralEstimateError("power iteration on J was not finite")
        return out, float("nan")

    def current_gain_floor(self, epoch=None):
        """
        The floor in force this epoch.

        Ramped from the gain the network was initialised at up to `gain_floor`
        over `gain_floor_warmup_epochs`. Without the ramp the very first
        projection multiplies every magnitude by whatever factor closes the gap
        in one step -- on the real connectome that was a ~4x jump at epoch 0
        (init gain 0.25 at the trial's operating point against a floor of 0.95),
        which lands the network at an arbitrary point and gives the readout
        calibration nothing to track.
        """
        if self.gain_floor is None:
            return None
        w = self.gain_floor_warmup_epochs
        if epoch is None or not w or epoch >= w:
            return self.gain_floor
        g0 = self._gain_at_init if self._gain_at_init is not None else self.gain_floor
        frac = float(epoch) / float(w)
        return min(self.gain_floor, g0 + frac * (self.gain_floor - g0))

    @torch.no_grad()
    def project_gain(self, D, epoch=None, iters=30):
        """
        Projected-gradient step onto `gain(D) in [floor, ceiling]`.

        Hard, because neither bound survives as a penalty:
        ...
        * **Ceiling.** Everything interesting lives in the fourth decimal place
          ...
          epoch is NaN.
        * **Floor.** Measured, a floor penalty at strength 1.0 contributed 0.19
          to the loss while the shape term was 0.65 and falling -- the shape
          gradient simply paid it and the gain still slid 0.64 -> 0.49.

        **Both bounds are evaluated at the SAME `D`.** An earlier version put the
        ceiling at the trial's peak operating point and the floor at its mean,
        which is infeasible for any real trajectory: it requires
        `<D_peak>/<D_mean> <= ceiling/floor`, i.e. about 1.05, while a driven
        network easily spans 3x. The projection then bounced up and down between
        the two bounds every epoch and the run died. `D` here is the peak slope,
        because that is the one stability depends on; the gain at the mean
        operating point is lower and is reported separately rather than
        constrained.

        Returns (gain after projection, whether it acted).
        """
        floor = self.current_gain_floor(epoch)
        acted = False

        if self.recurrent_gain(D=D) > self.gain_target:
            self._rescale_to_gain(self.gain_target, D=D, iters=iters, quiet=True)
            acted = True
        elif floor is not None and self.recurrent_gain(D=D) < floor:
            self._rescale_to_gain(floor, D=D, iters=iters, quiet=True)
            acted = True

        return self.recurrent_gain(D=D), acted
    ...
    def spectral_penalty(self, D=None, epoch=None):
        ...
        if floor is not None:
            pen = pen + self.spectral_penalty_strength * torch.relu(floor - gain).pow(2)
    ...
    def fit(self, train_list, x0=None, n_epochs=1000, verbose=True,
        ...
        for epoch in range(n_epochs):
            ...
            # The whole epoch runs inside one recovery block. A non-finite
            # trajectory, a numerically singular eigenvector basis, or a
            # non-finite penalty all land here rather than reaching
            # `loss.backward()` -- one NaN gradient poisons W permanently, and
            # the eig call on the NaN W that follows aborts inside oneMKL and
            # kills the process instead of raising.
            try:
                x_pred, y_pred_full = self.forward(x0, inputs, filter_xs=False)
                if not torch.isfinite(x_pred).all():
                    raise SpectralEstimateError("non-finite trajectory")
                ...
                # Operating point from this epoch's own trajectory: the gain
                # that matters is the one at the h the network actually visits,
                # not the one at h = 0. The PEAK slope is what the projection
                # uses; this mean one is what gets reported.
                D_now = self.operating_point_slope(x_pred.detach().mean(dim=(0, 1)))
                spec_pen, mu_abs, gain, imag_frac = self.spectral_penalty(D=D_now,
                                                                          epoch=epoch)
                loss = loss + spec_pen
                ...
                if not torch.isfinite(loss):
                    raise SpectralEstimateError("non-finite loss")

            except SpectralEstimateError as exc:
                n_nonfinite += 1
                if verbose:
                    print(f"[{epoch:5d}] recovering ({exc}): restoring the best state, "
                          f"projecting the gain, halving the lr.")
                with torch.no_grad():
                ...
                if n_nonfinite > 10:
                    print("Aborting: 10 recoveries. Lower lr, lower init_rho_target, "
                          "or widen the gain band (raise tau or tau_eff_min).")
                    break
                continue
            ...
            if verbose and (epoch % self.verbose_every == 0
                ...
                _fl = self.current_gain_floor(epoch)
                band = ("lo" if (_fl is not None and gain < _fl - 1e-4)
            ...
            if project_gain:
                ...
                _, acted = self.project_gain(D_peak, epoch=epoch)
        ...
        if verbose:
            print(f"\n[fit] gain projections: {n_projections}/{epoch + 1} epochs | "
                  f"recoveries: {n_nonfinite}")
            if n_projections > 0.9 * (epoch + 1) and self.gain_floor is not None:
                print("[fit] the gain bound was active on almost every epoch: what you "
                      "have is\n      the best this connectome can do WHILE "
                      "integrating, not an unconstrained\n      optimum. Re-run with "
                      "tau_eff_min=None to see the unconstrained one.")
    ...
    def report(self, train_list, x0=None, downsample_target_list=None,
        ...
        try:
            self._require_finite(J)
        except SpectralEstimateError:
            print("\n=== RNNConnectomeV2 report ===")
            print("W is not finite -- nothing to report. The run ended in a bad state.")
            return {"r2": r2, "eigvals": None}
        lam = torch.linalg.eigvals(J)
        ...
        print("per-population R^2: " + "  ".join(f"{lab} {v:5.2f}"
                                                 for lab, v in zip(pop_labels, r2)))
        D_mean = self.operating_point_slope(self.xs.detach().mean(dim=(0, 1))
                                            if self.xs is not None else None)
        D_peak = self.operating_point_slope(self.xs.detach().amax(dim=(0, 1))
                                            if self.xs is not None else None)
        g_mean, g_peak = self.recurrent_gain(D=D_mean), self.recurrent_gain(D=D_peak)
        print(f"recurrent gain rho(D W_fast): {g_mean:.4f} at the mean operating point "
              f"(tau_eff {self.tau / max(1e-9, 1 - g_mean):.2f} s), "
              f"{g_peak:.4f} at the peak (tau_eff "
              f"{self.tau / max(1e-9, 1 - g_peak):.2f} s)")
        print(f"  band (enforced at the peak): "
              f"[{'none' if self.gain_floor is None else format(self.gain_floor, '.4f')}, "
