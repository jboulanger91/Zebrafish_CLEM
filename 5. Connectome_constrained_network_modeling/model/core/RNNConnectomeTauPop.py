...
Additionally one value for tau is learned per-population. The orders of
magnitudes in the learning range are kept consistent with other values reported
in the literature: from 0.001 to 0.2s.
...
class RNNConnectomeTauPop(nn.Module):
    def __init__(
            self,
            dict_neurons,
            W_fixed=None,
            input_dim=1,
            tau=0.1, dt=0.01,
            learn_tau=True,             # fit one time constant per population
            tau_min=0.001,              # hard floor for learned tau (seconds)
            tau_max=0.20,               # hard ceiling for learned tau (seconds)
            lr=1e-3,
            weight_decay=1e-5,
        ...
        # ---- integration constants & learned timescale setup -----------------
        self.dt = dt
        self.tau_default = float(tau)
        self.alpha = dt / tau  # dimensionless step
        ...
        self.n_pops = len(self.population_indices)

        # Mapping from unit index to population index (0..n_pops-1)
        neuron_to_pop = torch.full((self.n_units,), -1, dtype=torch.long)
        for k, idx in enumerate(self.population_indices):
            if len(idx) > 0:
                neuron_to_pop[torch.as_tensor(idx, dtype=torch.long)] = k
        self.register_buffer("neuron_to_pop", neuron_to_pop, persistent=False)

        # Learnable tau per population, bounded in [tau_min, tau_max] via sigmoid(eta_tau)
        tau_clamped = min(self.tau_max - 1e-4, max(self.tau_min + 1e-4, self.tau_default))
        s0 = (tau_clamped - self.tau_min) / (self.tau_max - self.tau_min)
        s0 = min(1.0 - 1e-4, max(1e-4, s0))
        eta_tau0 = float(np.log(s0 / (1.0 - s0)))
        self.eta_tau = nn.Parameter(eta_tau0 * torch.ones(self.n_pops))
        ...
        tau_s = dt / self.alpha
        print(f"  default tau {self.tau_default:.3f} s "
              f"(learnable range [{self.tau_min}, {self.tau_max}]) | dt {self.dt} s")
    ...
    # ==================================================================
    # timescale (tau) methods
    # ==================================================================
    def taus(self):
        """
        Return learned tau (seconds) for each of the 8 populations,
        guaranteed to stay strictly inside [tau_min, tau_max].
        """
        s = torch.sigmoid(self.eta_tau)
        return self.tau_min + (self.tau_max - self.tau_min) * s

    def neuron_taus(self, device=None):
        """
        Broadcast learned population taus to individual neurons (n_units,).
        Neurons with unassigned population receive the baseline default tau.
        """
        pop_t = self.taus()
        if device is not None:
            pop_t = pop_t.to(device)
        dev = pop_t.device
        default_t = torch.tensor(self.tau_default, device=dev, dtype=pop_t.dtype)
        extended_tau = torch.cat([pop_t, default_t.unsqueeze(0)])
        mapping = torch.where(self.neuron_to_pop >= 0, self.neuron_to_pop, self.n_pops).to(dev)
        return extended_tau[mapping]

    def betas(self, device=None):
        """
        Compute per-neuron Euler-exponential integration factors:
        beta = exp(-dt / tau_i)
        one_minus_beta = 1 - beta
        """
        if not self.learn_tau:
            b = torch.exp(torch.tensor(-self.alpha, device=device))
            return b, 1.0 - b
        tau_vec = self.neuron_taus(device=device)
        beta = torch.exp(-self.dt / (tau_vec + 1e-12))
        return beta, 1.0 - beta

    def population_taus(self):
        """Convenience dictionary of population names to learned tau in seconds."""
        names = ["L_iMI", "L_cMI", "L_MON", "L_sMI", "R_iMI", "R_cMI", "R_MON", "R_sMI"]
        t_vals = self.taus().detach().cpu().tolist()
        return {names[i] if i < len(names) else f"pop_{i}": float(t_vals[i]) for i in range(len(t_vals))}
    ...
    def forward(self, x0, inputs, filter_xs=True):
        ...
        # Per-neuron exponential-Euler integration factor
        beta, one_minus_beta = self.betas(device)
